...
def get_files_ordered_by_date(folder_path: str) -> List[str]:
    ...
    files = [
        ...
        if os.path.isfile(os.path.join(folder_path, f)) & f.endswith(".mp4")
    ]
...
def size_of_files(file_list: List[str]) -> float:
    """
    Calculate the total size of the files in the list.
    """
    total_size = 0
...
def get_video_bitrate(input_file: str) -> int:
    ...
    Raises:
        VideoProcessingError: If there's an error getting the video bitrate.
    """
    try:
        ...
        raise VideoProcessingError(
            f"Failed to get video bitrate for {input_file}: {e.stderr.decode()}"
        ) from e
...
def get_audio_bitrate(input_file: str) -> int:
    ...
    Raises:
        VideoProcessingError: If there's an error getting the audio bitrate.
    """
    try:
        # Probe the input file to get its information
        probe = ffmpeg.probe(
            input_file, v="error", select_streams="a:0", show_entries="stream=bit_rate"
        )

        # Ensure 'streams' and 'bit_rate' exist in the probe result
        if (
            "streams" in probe
            and len(probe["streams"]) > 0
            and "bit_rate" in probe["streams"][0]
        ):
            bitrate = int(
                probe["streams"][0]["bit_rate"]
        ...
        raise VideoProcessingError(
            f"Failed to get audio bitrate for {input_file}: {e.stderr.decode()}"
        ) from e
